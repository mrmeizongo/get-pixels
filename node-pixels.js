...
exports.getPixels2 = function getPixels2(url, type) {
	return new Promise((resolve, reject) => {
		if (Buffer.isBuffer(url)) {
			if (!type) {
				reject(new Error("Invalid file type"));
			}
			doParse(type, url, resolve);
		} else if (url.indexOf("data:") === 0) {
			try {
				var buffer = parseDataURI(url);
				if (buffer) {
					process.nextTick(function () {
						doParse(type || buffer.mimeType, buffer.data, resolve);
					});
				} else {
					process.nextTick(function () {
						reject(new Error("Error parsing data URI"));
					});
				}
			} catch (err) {
				process.nextTick(function () {
